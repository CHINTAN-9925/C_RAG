...
- **Text PDFs only.** Scanned/image PDFs have no extractable text; add OCR if you
  need those.
- **Structured output** uses Groq `json_mode` with explicit key names plus Pydantic
  aliases, because Llama sometimes returns typed fields as strings or renames keys.
- **Everything is in-memory.** Rebuilding the index or restarting clears it; there
  is no persistent vector DB.
- To revert to the paid OpenAI stack, uncomment the lines marked `OpenAI (paid)` in
  `crag_engine.py` and set `OPENAI_API_KEY`.
